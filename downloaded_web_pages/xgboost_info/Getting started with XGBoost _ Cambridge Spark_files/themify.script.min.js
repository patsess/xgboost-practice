...
p.find(".tf-video")[0].play()},onLeave:function(c,d,p){b.removeClass("fullpagescroll-up fullpagescroll-down").addClass("fullpagescroll-"+p);if(0<b.find("> .mfp-wrap").length)return!1;var m=v?a(w).children(".section-container"):a(w).find(x);if(0<m.length&&0<c&&0<d)if("up"===p)for(;c>=d;c--)m.eq(c-1).find(".module_row").css("visibility","visible");else for(;c<=d;c++)m.eq(c-1).find(".module_row").css("visibility","visible")},afterSlideLoad:function(b,c,d,r){$aSectionHref=a("#main-nav").find('a[href$="#'+
b+"/"+d+'"]');0<$aSectionHref.length?$aSectionHref.closest("li").addClass("current_page_item").siblings().removeClass("current_page_item current-menu-item"):a("#main-nav").find("li").removeClass("current_page_item current-menu-item");history.replaceState(null,null,"#"+b+"/"+d);b=a(x+".active",a(w));b.find(".tf-video").length&&b.find(".tf-video")[0].play();Themify.body.trigger("themify_onepage_afterload")},onSlideLeave:function(b,c,d,r,f,g){b=a(".section-container").find(".module_row_slide");Themify.body.trigger("themify_onepage_slide_onleave",
[b.eq(f)]);if("left"===r)for(;d>f;d--)b.eq(d-1).css("visibility","visible");else if("right"===r)for(;d<f;d++)b.eq(d+1).css("visibility","visible")}})}function B(b,c){if("undefined"!==typeof a.fn.infinitescroll){var d=parseInt(themifyScript.maxPages);"undefined"!==typeof qp_max_pages&&(d=qp_max_pages);b.infinitescroll({navSelector:"#load-more a:last",nextSelector:"#load-more a:last",itemSelector:c,loadingText:"",donetext:"",loading:{img:themifyScript.loadingImg},maxPage:d,behavior:"auto"!==themifyScript.autoInfinite?
"twitter":"",pathParse:function(a){return a.match(/^(.*?)\b\d+\b(?!.*\b\d+\b)(.*?$)/).slice(1)},bufferPx:parseInt(themifyScript.bufferPx),pixelsFromNavToBottom:a("#footerwrap").height(),state:{currPage:themifyScript.currentPage&&Number.isInteger(+themifyScript.currentPage)?+themifyScript.currentPage:1}},function(c,e,g){var f=a(c);a(".newItems").removeClass("newItems");f.addClass("newItems");"reset"===themifyScript.resetFilterOnLoad&&LayoutAndFilter.reset();f.hide().imagesLoaded(function(){f.fadeIn();
